...
"use client";
import { MoreHorizontal, ShieldAlert, UserCircle2 } from "lucide-react";
...
export function MembersTable({
  ...
  // Update the Table component to be responsive
  return (
            ...
            {isLoading ? (
              ...
              Array.from({ length: 5 }).map((_, index) => (
                ...
                >
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-3">
                      <Skeleton className="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-zinc-800 shrink-0" />
                      <div className="space-y-1 min-w-0">
                        <Skeleton className="h-4 w-24 sm:w-32 bg-zinc-800" />
                        <Skeleton className="h-3 w-28 sm:w-40 bg-zinc-800" />
                      </div>
                    </div>
                  </TableCell>
                    ...
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <Avatar className="h-8 w-8 sm:h-10 sm:w-10 border border-zinc-800 shrink-0">
                          <AvatarImage src={member.image as string} />
                          <AvatarFallback className="bg-zinc-800 text-zinc-200 text-xs sm:text-sm">
                            {getInitials(member.name)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="min-w-0 truncate">
                          <div className="font-medium text-zinc-200 text-sm sm:text-base truncate">
                            {member.name}{" "}
                            {isCurrentUser && (
                          ...
                          {/* Mobile-only role and status indicators */}
                          <div className="flex items-center gap-2 sm:hidden mt-1">
                            ...
                            <Badge
                              variant="outline"
                              className="bg-emerald-500/10 hidden md:block text-emerald-500 border-emerald-500/20 text-xs py-0 h-5"
                            >
                              Active
                            </Badge>
                          </div>
